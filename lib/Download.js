...
Download.prototype.start = function() {
	...
	this.options.onEnd = function(err, result) {
        ...
        // If we encountered an error and it's not an "Invalid file path" & "Start a new download" error, we try to resume download "maxRetries" times
        if(err && (''+err).indexOf('Invalid file path') == -1 &&
            (''+err).indexOf('Start a new download') == -1 &&
            self._retryOptions._nbRetries < self._retryOptions.maxRetries
        ) {
            self.setStatus(2);
